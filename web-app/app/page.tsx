...
export function SwipeButtons({ onJoin, onSkip, onBack, isFirstCard, isLastCard }: SwipeButtonsProps) {
    return (
        <div className="flex justify-center items-center pt-6 gap-4">
            ...
            <Button onClick={onSkip} variant="destructive" disabled={isLastCard}>
                Skip
            </Button>
...
const cardVariants = {
  ...
  exit: (custom: { direction: number, action: 'join' | 'skip' | 'back' }) => ({
    zIndex: 0,
    x: custom.direction < 0 ? 1000 : -1000,
    opacity: 0,
    rotate: custom.direction < 0 ? 45 : -45,
    backgroundColor: 
        custom.action === 'join' ? '#22c55e' // Green for "Join"
      : custom.action === 'skip' ? '#ef4444' // Red for "Skip"
...
export default function Home() {
    ...
    useEffect(() => {
        try {
            window.localStorage.setItem('clubStatuses', JSON.stringify(clubStatuses));
        } catch (error) {
    ...
    const handleJoin = () => {
        ...
        setClubStatuses(prevStatuses => ({ ...prevStatuses, [currentClubName]: 'joined' }));
    ...
    const handleSkip = () => {
        ...
        const currentClubName = clubInfo[currentIndex]["Club Name"];
        setClubStatuses(prevStatuses => ({ ...prevStatuses, [currentClubName]: 'skipped' }));
    ...
    const status = !hasFinished ? clubStatuses[currentClub["Club Name"]] : undefined;
    ...
    return (
        <>
            ...
            <div className="flex justify-center items-start h-screen py-8" style={{ position: 'relative', overflow: 'hidden' }}>
                ...
                {hasFinished ? (
                    ...
                    <AnimatePresence initial={false} custom={{ direction, action }}>
