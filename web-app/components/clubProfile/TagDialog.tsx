...
export default function ContactDialog({ data }: ContactDialogProps) { 
    ...
    const removeTag = (itemToRemove: string) => {
        setTempTags(prevItems => prevItems.filter(item => item !== itemToRemove));
    };
            ...
            <Dialog open={isOpen} onOpenChange={handleOpenChange}>
                <DialogTrigger asChild>
                    <div className="rounded-full cursor-pointer bg-gray-200 hover:bg-gray-100 min-h-5 max-w-30 whitespace-pre-wrap hover:shadow-sm py-1 text-center">
                        <label className="text-sm font-medium cursor-pointer px-2">{title}</label>
                    </div>
                </DialogTrigger>
                <DialogTrigger asChild>
                    <div className="flex flex-wrap gap-2">
                        {tempTags.map((tag) => (
                            <label key={tag} className="rounded-full border border-gray-500 min-h-5 whitespace-pre-wrap shadow-sm py-1 px-1 text-center text-sm">
                            {tag}
                            </label>
                        ))}
                    </div>
                ...
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Make changes to your organization&apos;s tags here. Click save when you&apos;re done.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="flex flex-col">
                        <div>
                            ...
                            <div className="flex flex-wrap pt-4 gap-2 mb-6">
                                {tempTags.length === 0 ? (
                                    ...
                                    tempTags.map((tag) => (
                                        <label key={tag} className="rounded-full border border-gray-500 hover:bg-red-50 hover:text-red-600 hover:border-red-600 shadow-sm min-h-5 whitespace-pre-wrap py-1 px-1 text-center text-sm" onClick={() => removeTag(tag)}>
                                            {tag}
                                        </label>
                                ))
                                )}
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap pt-4 gap-2 mb-6">
                                {tagOptions.map((tag) => (
                                    <button key={tag} className="rounded-full border border-gray-500 hover:bg-gray-300 shadow-sm min-h-5 whitespace-pre-wrap py-1 px-1 text-center text-sm" onClick={() => toggleTag(tag)}>
                                        {tag}
                                    </button>
                            ))}
                            </div>
